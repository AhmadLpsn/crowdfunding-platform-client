...
const loadKeyPairs = (path: string) => {
    ...
    console.log(keyPairs.publicKey.toString())
}
...
const createAccount = async (seed: string, space: number, connection: Connection) => {
    ...
    const hash = await connection.sendTransaction(transaction,[keyPairs])
    console.log(hash);
    const result = await connection.confirmTransaction(hash);
    console.log(result)
}
loadKeyPairs('./keys.json')
establishConnection("https://8899-ahmadlpsn-crowdfundingpl-h8n7fc3l7t3.ws-eu40.gitpod.io/")

    .then(connection => createAccount("campaign 7", 1024, connection))
    .catch(console.log) 
